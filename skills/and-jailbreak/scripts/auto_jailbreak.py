...
Usage in execute_code:
    # Load the kit first (loader resolves scripts dir automatically):
    #   exec(open(<path-to>/scripts/load_godmode.py).read())
...
try:
    ...
    for _cand in [
        _hermes_home / "skills" / "red-teaming" / "godmode",
        _hermes_home / "skills" / "godmode",
        _hermes_home / "skills" / "and-jailbreak",
    ]:
        if (_cand / "scripts" / "parseltongue.py").exists():
            _SKILL_DIR = _cand
            break
    else:
        _SKILL_DIR = _hermes_home / "skills" / "red-teaming" / "godmode"
...
def _and_load_sibling(_path):
    """Exec a sibling script into its own namespace with __name__ set to a
    non-main value so argparse CLI guards never fire (fixes standalone CLI)."""
    _ns = dict(_caller_globals)
    _ns["__name__"] = "_and_jb_sibling"
    _ns["__file__"] = str(_path)
    exec(compile(open(_path).read(), str(_path), 'exec'), _ns)
    for _k, _v in _ns.items():
        if _k.startswith('_and_'):
            continue
        if callable(_v) or _k.isupper():
            _caller_globals[_k] = _v

if _parseltongue_path.exists():
    _and_load_sibling(_parseltongue_path)
if _race_path.exists():
    _and_load_sibling(_race_path)
